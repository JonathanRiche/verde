...
### Building directly (lower-level command)

`mise run build` / `mise run dev` wrap the correct invocation. If you must call the build yourself, use:

```bash
zig build --release=safe -Dbrowser-backend=native_webview
```

Bare `zig build` does **not** compile: it defaults to Debug + the WPE `verde-browser-linux` helper, which trips a vendored `packages/ghostty/.../osc.zig` comptime `@sizeOf` assertion and a `crt1.o` linker error. Run it from the **repo root** — running it from `packages/desktop` installs to `packages/desktop/zig-out`, not the top-level `zig-out/bin/verde` the app launches.

Never run `zig build run`, `mise run dev*`, or `pkill verde` from inside a Verde terminal pane: the agent runs in the Verde daemon, so those kill the daemon (and your own session). Only ever `zig build`; let the user relaunch to test UI changes.

### Zig 0.16 std reorganization (the I/O overhaul)

Zig 0.16 reorganized large parts of `std` around the new `std.Io` interface
("Writergate") and removed the medium-level `std.posix` file abstraction — all
I/O now takes an `Io` instance. This is upstream Zig, not anything this repo
removed; older docs/snippets that predate 0.16 won't compile. When unsure, run
`zigdoc <symbol>`. The ones you'll hit here:

- **`std.posix` file ops removed** (`open`/`close`/`read` as a "medium-level"
  layer). Go higher (`std.Io`) or lower (`std.posix.system` / `std.c`). This
  codebase uses the low-level route: `std.posix.openat(std.posix.AT.FDCWD, …)`
  (`openat` survived) + `std.c.close(fd)`; `std.posix.read` still exists.
- **`std.fs.File` → `std.Io.File`, `std.fs.Dir` → `std.Io.Dir`**, and FS ops now
  take an `Io`: `file.close()` → `file.close(io)`, `file.read` →
  `file.readStreaming`. Get one via
  `var threaded: std.Io.Threaded = .init(allocator, .{}); const io = threaded.io();`.
- **`std.fs.selfExePathAlloc` → `std.process.executablePathAlloc`**
  (`selfExeDirPath` → `std.process.executableDirPath`). Prefer that for new code;
  a couple of files still carry a local `/proc/self/exe` readlink helper.
- **Writer/Reader:** `std.fmt.format` → `std.Io.Writer.print`;
  `Io.GenericReader` / `AnyWriter` / `null_writer` are gone.
- **`std.mem.trimLeft` / `trimRight` → `std.mem.trimStart` / `trimEnd`.**
- **Containers are unmanaged by default** (`ArrayList`, hashmaps): init `.empty`,
  pass the allocator to ops (`list.append(allocator, x)`). See "Common Zig
  Patterns" above.

`zig ast-check <file>` catches syntax/local errors quickly but NOT these
moved-symbol errors — always confirm with a full build.
