...
- When retrying a failed, unpublished release, reuse its existing version/tag after fixing and verifying the failure; update that draft’s release notes. Do not create a new version solely because CI failed. Never move a successfully published release tag.
- SDK/dependency changes must keep `package.json`, `package-lock.json`, and `bun.lock` synchronized wherever both lockfiles are tracked. Release CI uses Bun's lockfile; an npm-only update is incomplete.
- Before tagging a release, run the workflow's frozen dependency installs from clean temporary copies of the root and web package manifests/lockfiles, using the Bun versions configured in `.github/workflows/release.yml`. Verify `bun install --frozen-lockfile --production` at the root and `bun install --frozen-lockfile` for `packages/web_app`; neither may change its lockfile. Existing `node_modules`, successful builds, and provider tests do not replace this check. Preserve the developer's installed dependencies and include SDK/version changes and release fixes in the release notes.
...
- [Android app](packages/mobile_android/AGENTS.md)
