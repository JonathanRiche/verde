...
pub const Profile = struct {
    ...
    pub fn createLocal(
        ...
        const id = try generateIdAlloc(allocator, random);
    ...
    pub fn createPairedSshTunnel(
        ...
        var created = try createSshTunnel(allocator, random, label, null, input);
...
/// Generates an opaque profile identity from a caller-supplied randomSecure
/// generator. The platform owns entropy acquisition; the caller owns the slice.
pub fn generateIdAlloc(allocator: std.mem.Allocator, random: anytype) ![]u8 {
    var random_bytes: [PROFILE_ID_RANDOM_BYTES]u8 = undefined;
    try random.randomSecure(&random_bytes);
...
const TestRandom = struct {
    ...
    pub fn randomSecure(self: *TestRandom, bytes: []u8) !void {
...
test "owned profiles sanitize fields and round trip stable ids" {
    ...
    var local = try Profile.createLocal(allocator, &random, "  Local  ", null);
...
test "current version persists paired-device and connect access without secrets" {
    var random: TestRandom = .{};
    const allocator = std.testing.allocator;
    var paired = try Profile.createPairedSshTunnel(allocator, &random, "Paired VM", .{ .host = "vm.example" });
    ...
    var linked = try Profile.createConnect(allocator, &random, "Connect", " https://connect.example/ ");
