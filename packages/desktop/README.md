...
Config supports UI and terminal font size, scrolling-pane activation and spacing, keybind overrides, and the default action behind the main `Open` button plus `Alt+O`. Settings can override scrolling mode and threshold for the currently selected workspace; clearing the override returns it to these global values.
