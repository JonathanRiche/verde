...
extern fn verde_windows_webview2_set_bounds(handle: ?*anyopaque, x: c_int, y: c_int, width: c_int, height: c_int) c_int;
extern fn verde_windows_webview2_navigate(handle: ?*anyopaque, url: [*:0]const u8) c_int;
extern fn verde_windows_webview2_eval(handle: ?*anyopaque, js: [*:0]const u8) c_int;
extern fn verde_windows_webview2_post_json(handle: ?*anyopaque, json: [*:0]const u8) c_int;
extern fn verde_windows_webview2_go_back(handle: ?*anyopaque) c_int;
extern fn verde_windows_webview2_go_forward(handle: ?*anyopaque) c_int;
...
pub const Controller = struct {
    ...
    pub fn deinit(self: *Controller) void {
        if (self.handle) |handle| {
            verde_windows_webview2_destroy(handle);
            self.handle = null;
        }
    }
    ...
    pub fn isRuntimeInitialized(self: *const Controller) bool {
        return self.handle != null and verde_windows_webview2_is_ready(self.handle) != 0;
    }
    ...
    /// WebView2 renders in-window; no CPU frame is available to copy yet.
    ...
    pub fn show(self: *Controller) !void {
        ...
        if (verde_windows_webview2_show(handle) == 0) return error.BrowserUnavailable;
    }

    pub fn hide(self: *Controller) !void {
        if (self.handle) |handle| {
            if (verde_windows_webview2_hide(handle) == 0) return error.BrowserUnavailable;
        }
    }
    ...
    pub fn navigate(self: *Controller, url: []const u8) !void {
        ...
        if (verde_windows_webview2_navigate(handle, z.ptr) == 0) return error.NavigateFailed;
    }
    ...
    pub fn eval(self: *Controller, js: []const u8) !void {
        ...
        defer self.allocator.free(z);
        if (verde_windows_webview2_eval(handle, z.ptr) == 0) return error.EvalFailed;
    }
    ...
    pub fn postJson(self: *Controller, json: []const u8) !void {
        ...
        defer self.allocator.free(z);
        if (verde_windows_webview2_post_json(handle, z.ptr) == 0) return error.EvalFailed;
    }
    ...
    pub fn reload(self: *Controller) !void {
        const handle = try self.ensureWebView();
        if (verde_windows_webview2_reload(handle) == 0) return error.BrowserUnavailable;
    }

    pub fn focus(self: *Controller) !void {
        const handle = try self.ensureWebView();
        if (verde_windows_webview2_focus(handle) == 0) return error.BrowserUnavailable;
    }

    pub fn blur(self: *Controller) !void {
        if (self.handle) |handle| {
            if (verde_windows_webview2_blur(handle) == 0) return error.BrowserUnavailable;
        }
    }
    ...
    pub fn popEvent(self: *Controller) ?browser_types.Event {
        ...
        if (verde_windows_webview2_pop_event(handle, &kind_int, &payload_ptr) == 0) return null;
        defer verde_windows_webview2_free_string(payload_ptr);
    ...
    fn applyBounds(self: *Controller, handle: *anyopaque) !void {
        if (verde_windows_webview2_set_bounds(
            handle,
