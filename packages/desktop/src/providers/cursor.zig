...
//! owns Cursor binary discovery, argv construction, auth/model discovery via
//! the Cursor CLI, and Cursor's model id suffix algebra.
...
pub const Client = struct {
    ...
    pub fn authState(self: *Client) !provider_types.AuthState {
        ...
        defer parsed.deinit();
        if (acp.getOptionalObjectBool(parsed.value, "isAuthenticated") orelse false) return .signed_in;
        if (acp.getOptionalObjectString(parsed.value, "status")) |status| {
    ...
    fn listThreadsAcp(self: *Client, allocator: std.mem.Allocator) ![]provider_types.ChatThreadSummary {
        var proc = try self.spawnAcp(allocator, null, false);
        defer proc.deinit();
    ...
    fn sendPromptAcp(
        ...
        var proc = try self.spawnAcp(allocator, model_arg, acp.shouldAutoApprovePermission(request));
        defer proc.deinit();
    ...
    fn spawnAcp(self: *Client, allocator: std.mem.Allocator, model_arg: ?[]const u8, force_commands: bool) !acp.Process {
        var env_map = try self.cursorEnvMap(allocator);
        errdefer env_map.deinit();
        const executable = try self.resolveExecutable(allocator, &env_map);
...
/// Maps provider-neutral ACP errors to Cursor's error names so existing
/// user-facing error messaging keeps working unchanged.
fn mapAcpError(err: anyerror) anyerror {
    return switch (err) {
        error.AcpFailed => error.CursorAcpFailed,
        error.AcpRefused => error.CursorAcpRefused,
        error.AcpSignedOut => error.CursorSignedOut,
        error.AcpMessageTooLarge => error.CursorAcpMessageTooLarge,
        error.AcpAttachmentsUnsupported => error.CursorAttachmentsUnsupported,
        else => err,
    };
}
...
fn cursorModelArgAlloc(allocator: std.mem.Allocator, model: []const u8, params_json: ?[]const u8) !?[]u8 {
    ...
    for (parsed.value.array.items) |item| {
        ...
        const value = acp.getOptionalObjectString(item, "value") orelse continue;
