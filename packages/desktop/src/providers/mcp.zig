...
const FX_MARKER = "verde-managed-mcp-v2\n";
...
fn installProviderAtHome(allocator: std.mem.Allocator, home: []const u8, endpoint: *const mcp_endpoint.Endpoint, provider: Provider) !void {
    return switch (provider) {
        ...
        .fx => installFxAtHome(allocator, home),
...
fn inspectProviderAtHome(allocator: std.mem.Allocator, home: []const u8, provider: Provider) RegistrationStatus {
    return switch (provider) {
        ...
        .fx => inspectFxAtHome(allocator, home),
...
fn inspectFxAtHome(allocator: std.mem.Allocator, home: []const u8) RegistrationStatus {
    return inspectManagedFileAtHome(allocator, home, ".fx/verde-mcp.enabled", FX_MARKER);
}

fn installFxAtHome(allocator: std.mem.Allocator, home: []const u8) !void {
    const path = try configPathAlloc(allocator, home, ".fx/verde-mcp.enabled");
    defer allocator.free(path);
    var threaded: std.Io.Threaded = .init(allocator, .{});
    defer threaded.deinit();
    const io = threaded.io();
    if (try readFileIfPresent(allocator, io, path)) |existing| {
        defer allocator.free(existing);
        if (!std.mem.eql(u8, existing, FX_MARKER)) return error.ProviderMcpConflict;
    }
    try writeFileAtomic(allocator, io, path, FX_MARKER);
}

fn removeFxAtHome(allocator: std.mem.Allocator, home: []const u8) !void {
    return removeManagedFileAtHome(allocator, home, ".fx/verde-mcp.enabled", FX_MARKER);
}
...
test "managed JSON registration preserves user servers and uninstalls only Verde" {
    ...
    const detected = [ALL_PROVIDERS.len]bool{ false, true, true, true, true, false, false, false };
