...
fn pollCoreChangesOnce(storage: *const Storage, loop: *ChangeCursorLoopState, cursor: u64) ChangeCursorPollOutcome {
    ...
    const refresh = fetchOwnedCompositeSnapshot(storage) orelse return .unavailable;
...
fn fetchCompositeSnapshotSeed(storage: *const Storage, loop: *ChangeCursorLoopState) bool {
    const refresh = fetchOwnedCompositeSnapshot(storage) orelse return false;
    if (!loop.publishRefresh(refresh)) {
        refresh.deinit();
        return false;
    }
...
pub const AppState = struct {
    ...
    pub const sendThreadDraftWithUiPolicy = chat_controller.sendThreadDraftWithUiPolicy;
    ...
    pub const storeThreadFollowupPrompt = chat_controller.storeThreadFollowupPrompt;
