...
fn fontForRole(role_fonts: RoleFonts, role: palette.FontRole) *palette.sdl.Font {
    return switch (role) {
        ...
        // mono_symbols / symbols / emoji are coverage fallbacks the renderer
        // picks per-glyph; measurement always reports the primary mono cell
        // width because terminal layout advances by mono cells regardless of
        // which face ends up drawing each glyph.
        .mono_symbols, .symbols, .emoji => role_fonts.mono,
    };
}
