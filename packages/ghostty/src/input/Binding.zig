...
pub const Action = union(enum) {
    ...
    pub fn Scoped(comptime s: Scope) type {
        ...
        var enum_names: [i][]const u8 = undefined;
        const TagInt = std.math.IntFittingRange(0, i);
        var enum_values: [i]TagInt = undefined;
        var union_names: [i][]const u8 = undefined;
        var union_types: [i]type = undefined;
        var union_attrs: [i]std.builtin.Type.UnionField.Attributes = undefined;
        for (enum_fields[0..i], union_fields[0..i], 0..) |enum_field, union_field, field_i| {
            enum_names[field_i] = enum_field.name;
            enum_values[field_i] = @intCast(enum_field.value);
            union_names[field_i] = union_field.name;
            union_types[field_i] = union_field.type;
            union_attrs[field_i] = .{ .@"align" = union_field.alignment };
        }

        const Tag = @Enum(TagInt, .exhaustive, &enum_names, &enum_values);
        return @Union(.auto, Tag, &union_names, &union_types, &union_attrs);
    }
