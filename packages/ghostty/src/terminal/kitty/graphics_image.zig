...
pub const LoadingImage = struct {
    ...
    pub fn init(
        ...
        const path = switch (t.medium) {
            ...
            .file, .temporary_file => blk: {
                const len = std.Io.Dir.realPathFileAbsolute(threaded.io(), cmd.data, &abs_buf) catch |err| {
    ...
    fn readSharedMemory(
        ...
        const stat_size: usize = max_size;
    ...
    fn readFile(
        ...
        defer if (medium == .temporary_file) {
            var threaded = std.Io.Threaded.init_single_threaded;
            std.Io.Dir.deleteFileAbsolute(threaded.io(), path) catch |err| {
                log.warn("failed to delete temporary file: {}", .{err});
            };
        };

        var file_io = std.Io.Threaded.init_single_threaded;
        var file = std.Io.Dir.cwd().openFile(file_io.io(), path, .{}) catch |err| {
            log.warn("failed to open temporary file: {}", .{err});
            return error.InvalidData;
        };
        defer file.close(file_io.io());

        // File must be a regular file
        if (file.stat(file_io.io())) |stat| {
            if (stat.kind != .file) {
        ...
        var buf_reader = file.reader(file_io.io(), &buf);
        const reader = &buf_reader.interface;
        if (t.offset > 0) {
            reader.discardAll(t.offset) catch |err| {
    ...
    fn isPathInTempDir(path: []const u8) bool {
        ...
        var buf: [std.fs.max_path_bytes]u8 = undefined;
        var threaded = std.Io.Threaded.init_single_threaded;
        if (std.Io.Dir.realPathFileAbsolute(threaded.io(), dir, &buf)) |real_dir_len| {
            if (std.mem.startsWith(u8, path, buf[0..real_dir_len])) return true;
        } else |_| {}
