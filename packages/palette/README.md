...
Palette is Verde's retained UI component package. It uses Zig, SDL3, SDL3_ttf,
and SDL_GPU.
...
`pkg-config` is required because the Palette build links Homebrew's SDL
...
Palette components are retained, but controls that expose `setBounds()` can be
laid out every frame. Use `palette.layout` for container-style layout instead of
...
```zig
palette.layout.applyFlex(container, config, items, .{ &input, &button });
...
`Checkbox`, and `Toggle`. For other components, call `palette.layout.flex()` or
`palette.layout.grid()` into a rect array and wire the rects manually once that
component exposes runtime bounds.

### Boxes, Padding, And Margins

Use `palette.layout.Box` when a panel/container needs margin and padding. Margin
...
const shell: palette.layout.Box = .{
    ...
    .padding = palette.layout.Edges.xy(16, 10),
};
...
```zig
palette.layout.Edges.all(8)      // top/right/bottom/left = 8
palette.layout.Edges.xy(12, 6)   // left/right = 12, top/bottom = 6
palette.layout.Edges.axis(12, 6) // same as xy()
```
...
palette.layout.applyFlex(
    panel_rect,
    .{ .direction = .row, .gap = 8, .padding = palette.layout.Edges.all(12) },
    .{
        palette.layout.FlexItem{ .basis_w = 180, .basis_h = 32, .grow = 1 },
        palette.layout.FlexItem.fixed(72, 32),
    },
    .{ &input, &send_button },
);
```
...
```zig
var rects: [3]palette.Rect = undefined;
palette.layout.flex(
    panel_rect,
    .{ .direction = .row, .wrap = true, .gap = 8 },
    &.{
        palette.layout.FlexItem.fixed(120, 32),
        palette.layout.FlexItem.fixed(120, 32),
        palette.layout.FlexItem.fixed(120, 32),
    },
    &rects,
...
```zig
const columns = [_]palette.layout.Track{ .{ .px = 96 }, .{ .fr = 1 }, .{ .fr = 1 } };
const rows = [_]palette.layout.Track{ .{ .px = 32 }, .{ .px = 32 } };
const cells = [_]palette.layout.GridItem{
    .{ .column = 0, .row = 0 },
    .{ .column = 1, .row = 0, .column_span = 2 },
};
var rects: [cells.len]palette.Rect = undefined;

try palette.layout.grid(
    allocator,
    panel_rect,
...
Run the visual layout lab from `packages/palette`:
...
renders it as texture-backed Palette image components. The image row shows native
...
Run the layout review example from `packages/palette`:
...
Palette owns text layout for retained controls. Components do not ask the host to
...
```zig
const metrics = palette.FontMetrics.fixed(14, 8, 18);

var prompt = try palette.textInput(.{}).init(allocator, "");
...
```zig
fn advance(context: ?*anyopaque, text: []const u8, offset: usize, font_size: f32) palette.FontAdvance {
...
const metrics: palette.FontMetrics = .{
    .font_size = 14,
    .line_height = 18,
    .ascent = 13,
    .descent = 5,
...
layout Palette used for hit testing, wrapping, cursor movement, selection, and
...
`palette.select()` is intentionally a flat dropdown. For menus that need child
submenus, use `palette.cascadeMenu()`.

```zig
const Menu = palette.cascadeMenu(.{
    .item_count = 3,
...
`palette.toolbar()` is a retained horizontal layout helper for inline command
...
```zig
const Bar = palette.toolbar(.{ .gap = 8 });
var bar = Bar.init();
bar.setBounds(toolbar_rect);

const items = [_]palette.ToolbarItem{
    palette.ToolbarItem.fixed(132, 32),
    palette.ToolbarItem.flexible(120, 32, 1),
    .{ .width = 36, .height = 36, .right_aligned = true },
};
var rects: [items.len]palette.Rect = undefined;
bar.layout(&items, &rects);
try bar.renderSeparators(allocator, &batch, &items, &rects);
```

For Verde-style command prompts, `palette.composerPrompt()` owns the retained
...
```zig
const Composer = palette.composerPrompt(.{
    .model_icon = "O",
    .model_label = "GPT-5.5",
    .reasoning_label = "Low",
    .fast_icon = "~",
...
Palette render commands carry renderer-neutral shape style. Hosts should consume
...
`palette.text()` can render styled inline spans without host-side text splitting.
Set spans on the retained Text instance and Palette emits laid-out `TextRun`s
with per-run color and optional font size.

```zig
const Message = palette.text(.{ .width = 520, .height = 24, .glyph_width = 8 });
...
Use `palette.virtualList()` for large or variable-height scrollable lists such as
...
const Chats = palette.virtualList(.{
    .item_count = 200,
    .item_label = rowLabel,
...
Palette image rendering is command-based, like text and rectangles. Components
emit a stable `TextureId`; the renderer resolves that ID to a backend texture.
This keeps Palette components portable across SDL debug rendering, Vulkan, and
...
```zig
const Avatar = palette.image(.{
    .source_width = 256,
    .source_height = 256,
    .fit = .cover,
});

var avatar = Avatar.init(palette.TextureId.init(1));
...
Palette also ships a reusable image decoder backed by bundled `stb_image` files
under `packages/palette/vendor`. It supports PNG, JPEG, BMP, and GIF as compiled
by `vendor/stb_image_impl.c`.

```zig
const loaded = try palette.ImageLoader.load("assets/avatar.png");
defer loaded.deinit();

const texture = try palette.sdl.createTextureFromImage(renderer, loaded);
defer palette.sdl.destroyTexture(texture);
```

If your executable uses `palette.ImageLoader`, link the bundled decoder from your
`build.zig`:

```zig
const palette_dep = b.dependency("palette", .{ .target = target, .optimize = optimize });
const palette_mod = palette_dep.module("palette");
exe.root_module.addImport("palette", palette_mod);

const palette_build = @import("path/to/packages/palette/build.zig");
palette_build.linkImageLoader(palette_dep, exe.root_module);
```

The retained UI API does not depend on a specific decoder. Apps can still load
textures through another pipeline and only hand Palette a `TextureId`.
...
var presenter = palette.renderer.sdlFontRendererWithTextures(
    renderer,
    font,
    16,
    &texture_store,
