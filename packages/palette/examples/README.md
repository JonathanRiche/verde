...
Run examples from `packages/palette`.
...
composer through `palette.RenderBatch` plus Palette's SDL presenter, so the
...
The layout lab opens an SDL3 window for `palette.layout`. Resize it to inspect
...
The layout review example is a CLI example for `palette.layout`. It computes a
...
The composer prompt review example is a CLI example for the retained
`palette.composerPrompt()` component. It validates rounded panel commands,
...
window, type multiline prompt text, open model/reasoning menus, toggle
fast/access, and hover/click the circular send button to inspect the actual
rendered UI.

Rendering is command-based. Components emit `palette.RenderBatch` commands, and
the labs use Palette's SDL presenter for local inspection.
