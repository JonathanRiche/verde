...
const Label = palette.text(.{ .x = 12, .y = 10, .width = 180, .height = 24, .selectable = true });
const Input = palette.textInput(.{ .x = 12, .y = 42, .width = 220, .height = 32, .placeholder_text = "Filter" });
const PrimaryButton = palette.button(.{ .x = 12, .y = 84, .width = 96, .height = 32, .label = "Apply" });
const Logo = palette.image(.{ .x = 128, .y = 84, .width = 64, .height = 32, .source_width = 64, .source_height = 32, .fit = .contain });
const Check = palette.checkbox(.{ .x = 12, .y = 126, .label = "Enabled" });
const Switch = palette.toggle(.{ .x = 12, .y = 158, .label = "Live" });
const Items = palette.listBox(.{ .x = 260, .y = 10, .width = 180, .height = 96, .item_count = 8 });
const Choice = palette.select(.{ .x = 260, .y = 124, .width = 180, .height = 30, .menu_height = 90, .item_count = 5 });
const Strip = palette.tabs(.{ .x = 12, .y = 204, .width = 300, .height = 32, .tab_count = 3 });
const Viewport = palette.scrollArea(.{ .x = 460, .y = 10, .width = 120, .height = 96, .content_height = 240 });
const Dialog = palette.modal(.{ .x = 120, .y = 80, .width = 220, .height = 120 });
const Popup = palette.menu(.{ .x = 460, .y = 124, .width = 120, .row_height = 24, .item_count = 4 });
const Grid = palette.table(.{ .x = 600, .y = 10, .width = 260, .height = 120, .row_count = 6, .column_count = 3 });
const Source = palette.codeView(.{ .x = 600, .y = 150, .width = 260, .height = 120 });
...
pub fn main() !void {
    ...
    var input = try Input.init(allocator, "palette");
    defer input.deinit(allocator);
    var button = PrimaryButton.init();
    var logo = Logo.init(palette.TextureId.init(99));
    ...
    palette.layout.applyFlex(
        ...
        .{
            palette.layout.FlexItem{ .basis_w = 80, .basis_h = 32, .grow = 1 },
            palette.layout.FlexItem.fixed(80, 32),
        },
        .{ &input, &button },
    );
    ...
    try palette.layout.applyGrid(
        ...
        .{
            palette.layout.GridItem{ .column = 0, .row = 0, .column_span = 2 },
            palette.layout.GridItem{ .column = 0, .row = 1 },
            palette.layout.GridItem{ .column = 1, .row = 1 },
        },
