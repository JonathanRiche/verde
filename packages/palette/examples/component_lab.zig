//! SDL visual lab for retained Palette components beyond TextArea.
...
const Search = palette.textInput(.{
    .x = 24,
    .y = 76,
    .width = 280,
    .height = 34,
...
const ApplyButton = palette.button(.{ .x = 320, .y = 76, .width = 104, .height = 34, .label = "Apply" });
const ToolButton = palette.iconButton(.{ .x = 436, .y = 76, .width = 34, .height = 34, .icon_inset = 9 });
const ReadyCheck = palette.checkbox(.{ .x = 24, .y = 130, .size = 20, .label = "Ready" });
const LiveToggle = palette.toggle(.{ .x = 160, .y = 129, .width = 42, .height = 22, .label = "Live" });
const Items = palette.listBox(.{ .x = 24, .y = 198, .width = 220, .height = 132, .padding_x = 8, .padding_y = 6, .row_height = 24, .item_count = 12 });
const Choice = palette.select(.{ .x = 300, .y = 198, .width = 200, .height = 32, .menu_height = 120, .padding_x = 8, .padding_y = 6, .row_height = 28, .item_count = 8 });
const Strip = palette.tabs(.{ .x = 24, .y = 366, .width = 360, .height = 34, .tab_width = 120, .tab_count = 3 });
const Panel = palette.scrollArea(.{ .x = 560, .y = 76, .width = 160, .height = 128, .content_height = 320, .background_color = .{ .r = 0.07, .g = 0.08, .b = 0.10, .a = 1.0 }, .scrollbar_width = 6 });
const Popup = palette.menu(.{ .x = 740, .y = 96, .width = 160, .row_height = 28, .item_count = 5 });
const Dialog = palette.modal(.{ .x = 260, .y = 150, .width = 340, .height = 180 });
const Grid = palette.table(.{ .x = 560, .y = 254, .width = 330, .height = 132, .header_height = 26, .row_height = 24, .column_width = 110, .row_count = 14, .column_count = 3 });
const Source = palette.codeView(.{ .x = 24, .y = 430, .width = 856, .height = 132, .glyph_width = 8, .line_height = 18 });
...
pub export fn palette_component_lab_main() c_int {
    run() catch |err| {
        std.debug.print("palette component_lab failed: {s}\n", .{@errorName(err)});
        return 1;
    };
    return 0;
}
...
pub fn run() !void {
    ...
    const window = try sdl.Window.create("Palette Component Lab", 920, 600, labWindowFlags());
    ...
    var state: State = .{};
    var header = try Header.init(allocator, "Palette Component Lab");
    defer header.deinit(allocator);
    header.setCallbacks(.{ .set_clipboard = setClipboard });

    var search = try Search.init(allocator, "palette");
    ...
    std.debug.print("palette component_lab started; close the window or press Ctrl+C to exit.\n", .{});
    defer std.debug.print("palette component_lab stopped.\n", .{});
    ...
    while (running) : (frame_index += 1) {
        ...
        var presenter = palette.renderer.sdlFontRenderer(renderer, font, LABEL_FONT_SIZE);
...
fn routeEvent(
    ...
    switch (event.type) {
        ...
        .mouse_button_down => {
            const point: palette.draw.Vec2 = .{ .x = event.button.x, .y = event.button.y };
...
fn drawBatch(renderer: *sdl.Renderer, commands: []const palette.draw.Command) !void {
...
fn drawDebugOverlay(
    presenter: *palette.renderer.SdlFontRenderer,
    state: State,
    header: Header,
    search: Search,
    checkbox: ReadyCheck,
...
fn drawListLabels(presenter: *palette.renderer.SdlFontRenderer, list: Items) !void {
    var index: usize = 0;
...
fn drawSelectLabels(presenter: *palette.renderer.SdlFontRenderer, select: Choice) !void {
    var index: usize = 0;
...
fn drawTabLabels(presenter: *palette.renderer.SdlFontRenderer, tabs: Strip) !void {
    _ = tabs;
...
fn drawPanelContent(presenter: *palette.renderer.SdlFontRenderer, panel: Panel) !void {
    var row: usize = 0;
    while (row < 14) : (row += 1) {
...
fn drawMenuLabels(presenter: *palette.renderer.SdlFontRenderer) !void {
...
fn drawTableText(presenter: *palette.renderer.SdlFontRenderer, table: Grid) !void {
...
fn drawCodeText(presenter: *palette.renderer.SdlFontRenderer, source: Source) !void {
...
fn commandColor(command: palette.draw.Command) [4]u8 {
...
fn debugText(presenter: *palette.renderer.SdlFontRenderer, x: f32, y: f32, text: []const u8, color: [4]u8) !void {
    try presenter.renderLine(x, y, text, colorFromBytes(color), LABEL_FONT_SIZE);
}

fn colorFromBytes(color: [4]u8) palette.draw.Color {
    return .{
...
fn handleApplyEvent(context: ?*anyopaque, event: palette.ButtonEvent) void {
...
fn handleToolEvent(context: ?*anyopaque, event: palette.ButtonEvent) void {
    const state: *State = @ptrCast(@alignCast(context orelse return));
    switch (event) {
...
fn handleMenuEvent(context: ?*anyopaque, event: palette.MenuEvent) void {
    const state: *State = @ptrCast(@alignCast(context orelse return));
    switch (event) {
...
fn updateWindowTitle(window: *sdl.Window, state: State, search: Search, checkbox: ReadyCheck, toggle: LiveToggle, list: Items, select: Choice, tabs: Strip, command_count: usize, frame_index: usize) void {
    ...
    const title = std.fmt.bufPrintZ(
        &title_buffer,
        "Palette Component Lab | text={d} ready={} live={} list={?} select={?} tab={d} clicks={d}/{d} commands={d} frame={d}",
