//! SDL visual lab for the Palette composer prompt visual model.
...
pub export fn palette_composer_prompt_lab_main() c_int {
    run() catch |err| {
        std.debug.print("palette composer_prompt_lab failed: {s}\n", .{@errorName(err)});
        return 1;
    };
    return 0;
}
...
pub fn run() !void {
    ...
    const window = try sdl.Window.create("Palette Composer Prompt Lab", 900, 560, labWindowFlags());
    ...
    std.debug.print("palette composer_prompt_lab started; close the window or press Ctrl+C to exit.\n", .{});
    defer std.debug.print("palette composer_prompt_lab stopped.\n", .{});
...
fn layoutComposer(window: *sdl.Window, composer: *Composer) !palette.Rect {
...
fn renderBackground(allocator: std.mem.Allocator, batch: *palette.RenderBatch, window: *sdl.Window, composer_rect: palette.Rect) !void {
    const size = try window.size();
    const window_rect: palette.Rect = .{ .w = @floatFromInt(size.w), .h = @floatFromInt(size.h) };
...
fn countBatch(batch: palette.RenderBatch) Counts {
...
fn drawFrameLabels(presenter: *palette.renderer.SdlFontRenderer, composer_rect: palette.Rect, prompt_bytes: usize, state: LabState, counts: Counts, frame_index: usize) !void {
    try label(presenter, composer_rect.x, 42, "Palette Composer Prompt Lab", .{ 235, 241, 248, 255 });
...
fn updateWindowTitle(window: *sdl.Window, prompt_bytes: usize, state: LabState, counts: Counts, frame_index: usize) void {
    ...
    const title = std.fmt.bufPrintZ(
        &title_buffer,
        "Palette Composer Prompt Lab | prompt={d} model={d} reasoning={d} fast={d} access={d} send={d} commands={d} frame={d}",
...
fn handleComposerEvent(context: ?*anyopaque, event: palette.ComposerPromptEvent) void {
