//! SDL visual lab for Palette runtime flex and grid layout.

const std = @import("std");
const builtin = @import("builtin");
const palette = @import("palette");
const sdl = palette.sdl;
...
const Provider = palette.select(.{ .height = 32, .menu_height = 108, .item_count = 4, .item_label = providerLabel });
const Model = palette.select(.{ .height = 32, .menu_height = 108, .item_count = 4, .item_label = modelLabel });
const Reasoning = palette.select(.{ .height = 32, .menu_height = 108, .item_count = 4, .item_label = reasoningLabel });
const InlinePreview = palette.image(.{ .width = 48, .height = 38, .source_width = 105, .source_height = 122, .fit = .contain, .tint = palette.Color.white });
const NativeImage = palette.image(.{ .source_width = 105, .source_height = 122, .fit = .none, .tint = palette.Color.white });
const ContainImage = palette.image(.{ .source_width = 105, .source_height = 122, .fit = .contain, .tint = palette.Color.white });
const CoverImage = palette.image(.{ .source_width = 105, .source_height = 122, .fit = .cover, .tint = palette.Color.white });
const StretchImage = palette.image(.{ .source_width = 105, .source_height = 122, .fit = .stretch, .tint = palette.Color.white });
const CropImage = palette.image(.{ .source_width = 105, .source_height = 122, .fit = .cover, .uv = .{ .x = 0.0, .y = 0.0, .w = 0.55, .h = 0.55 }, .tint = palette.Color.white });
const Prompt = palette.textInput(.{ .height = 38, .placeholder_text = "Resize the window, type here, and click controls" });
const Send = palette.button(.{ .icon_text = ">", .font_size = 18, .corner_radius = 999 });
const Stop = palette.button(.{ .label = "Stop" });
const Fast = palette.toggle(.{ .label = "Fast" });
const Access = palette.checkbox(.{ .label = "Tools" });
const ChipA = palette.button(.{ .label = "Margin" });
const ChipB = palette.button(.{ .label = "Padding" });
const ChipC = palette.button(.{ .label = "Wrap" });
const ChipD = palette.button(.{ .label = "Grow" });
const ChipE = palette.button(.{ .label = "Grid" });
const ChipF = palette.button(.{ .label = "Span" });
...
const LayoutRects = struct {
    ...
    grid_panel: palette.Rect,
    prompt_panel: palette.Rect,
    image_panel: palette.Rect,
    wrap_panel: palette.Rect,
    grid_content: palette.Rect,
    prompt_content: palette.Rect,
    image_content: palette.Rect,
    wrap_content: palette.Rect,
};
...
pub export fn palette_layout_lab_main() c_int {
    run() catch |err| {
        std.debug.print("palette layout_lab failed: {s}\n", .{@errorName(err)});
        return 1;
    };
    return 0;
}
...
pub fn run() !void {
    ...
    const window = try sdl.Window.create("Palette Layout Lab", 920, 680, labWindowFlags());
    ...
    const preview_image = try palette.ImageLoader.load(PREVIEW_IMAGE_PATH);
    ...
    var controls: Controls = .{
        ...
        .preview = InlinePreview.initWithSize(palette.TextureId.init(1), preview_w, preview_h),
        .image_native = NativeImage.initWithSize(palette.TextureId.init(1), preview_w, preview_h),
        .image_contain = ContainImage.initWithSize(palette.TextureId.init(1), preview_w, preview_h),
        .image_cover = CoverImage.initWithSize(palette.TextureId.init(1), preview_w, preview_h),
        .image_stretch = StretchImage.initWithSize(palette.TextureId.init(1), preview_w, preview_h),
        .image_crop = CropImage.initWithSize(palette.TextureId.init(1), preview_w, preview_h),
    ...
    std.debug.print("palette layout_lab started; close the window or press Ctrl+C to exit.\n", .{});
    defer std.debug.print("palette layout_lab stopped.\n", .{});
    ...
    while (running) : (frame_index += 1) {
        ...
        var presenter = palette.renderer.sdlFontRendererWithTextures(renderer, font, LABEL_FONT_SIZE, &texture_store, lookupTexture);
...
fn layoutControls(allocator: std.mem.Allocator, window: *sdl.Window, controls: *Controls) !LayoutRects {
    ...
    const window_rect: palette.Rect = .{ .w = @floatFromInt(size.w), .h = @floatFromInt(size.h) };
    const shell_box: palette.layout.Box = .{
        .rect = window_rect,
        .margin = palette.layout.Edges.all(12),
        .padding = palette.layout.Edges.all(16),
    };
    const content = shell_box.contentRect();
    var rows: [4]palette.Rect = undefined;
    palette.layout.flex(
        content,
        .{ .direction = .column, .gap = 18 },
        &.{
            palette.layout.FlexItem.fixed(0, 104),
            palette.layout.FlexItem.fixed(0, 70),
            palette.layout.FlexItem.fixed(0, 190),
            palette.layout.FlexItem{ .basis_h = 52, .grow = 1, .min_h = 52 },
        },
        &rows,
    );

    const grid_box: palette.layout.Box = .{ .rect = rows[0], .padding = palette.layout.Edges.xy(14, 32) };
    const prompt_box: palette.layout.Box = .{ .rect = rows[1], .padding = palette.layout.Edges.xy(14, 24) };
    const image_box: palette.layout.Box = .{ .rect = rows[2], .padding = palette.layout.Edges.xy(14, 34) };
    const wrap_box: palette.layout.Box = .{ .rect = rows[3], .padding = palette.layout.Edges.xy(14, 30) };
    ...
    try palette.layout.applyGrid(
        ...
        .{
            ...
            palette.layout.GridItem{ .column = 4, .row = 0 },
        },
    ...
    palette.layout.applyFlex(
        ...
        .{
            palette.layout.FlexItem{ .basis_w = 280, .basis_h = 38, .grow = 1, .min_w = 160 },
            palette.layout.FlexItem.fixed(48, 38),
            palette.layout.FlexItem.fixed(86, 38),
            palette.layout.FlexItem.fixed(86, 38),
        },
    ...
    palette.layout.applyFlex(
        ...
        .{
            palette.layout.FlexItem{ .basis_w = 138, .basis_h = 34, .margin = palette.layout.Edges.xy(6, 0) },
            palette.layout.FlexItem{ .basis_w = 148, .basis_h = 34, .margin = palette.layout.Edges.xy(6, 0) },
            palette.layout.FlexItem{ .basis_w = 120, .basis_h = 34, .margin = palette.layout.Edges.xy(6, 0) },
            palette.layout.FlexItem{ .basis_w = 130, .basis_h = 34, .margin = palette.layout.Edges.xy(6, 0) },
            palette.layout.FlexItem{ .basis_w = 110, .basis_h = 34, .margin = palette.layout.Edges.xy(6, 0) },
            palette.layout.FlexItem{ .basis_w = 120, .basis_h = 34, .margin = palette.layout.Edges.xy(6, 0) },
        },
...
fn renderLabChrome(allocator: std.mem.Allocator, batch: *palette.RenderBatch, rects: LayoutRects) !void {
...
fn renderControls(allocator: std.mem.Allocator, batch: *palette.RenderBatch, controls: *const Controls) !void {
...
fn outline(renderer: *sdl.Renderer, rect: palette.Rect, color: [4]u8) !void {
...
fn drawLabels(presenter: *palette.renderer.SdlFontRenderer, rects: LayoutRects, state: State, controls: Controls, frame_index: usize) !void {
    try label(presenter, rects.shell.x + 12, rects.shell.y + 10, "Palette Layout Lab", .{ 235, 241, 248, 255 });
...
fn label(presenter: *palette.renderer.SdlFontRenderer, x: f32, y: f32, text: []const u8, color: [4]u8) !void {
    try presenter.renderLine(x, y, text, colorFromBytes(color), LABEL_FONT_SIZE);
}

fn imageCaption(presenter: *palette.renderer.SdlFontRenderer, bounds: palette.Rect, content: palette.Rect, text: []const u8) !void {
    try label(presenter, bounds.x + 6, content.y + content.h + 6, text, .{ 214, 226, 242, 255 });
}

fn colorFromBytes(color: [4]u8) palette.Color {
    return .{
...
fn lookupTexture(context: ?*anyopaque, id: palette.TextureId) ?palette.renderer.SdlTexture {
...
fn handleSend(context: ?*anyopaque, event: palette.ButtonEvent) void {
...
fn handleStop(context: ?*anyopaque, event: palette.ButtonEvent) void {
    const state: *State = @ptrCast(@alignCast(context orelse return));
    switch (event) {
...
fn handleChip(context: ?*anyopaque, event: palette.ButtonEvent) void {
    const state: *State = @ptrCast(@alignCast(context orelse return));
    switch (event) {
...
fn updateWindowTitle(window: *sdl.Window, state: State, controls: Controls, frame_index: usize) void {
    ...
    const title = std.fmt.bufPrintZ(
        &title_buffer,
        "Palette Layout Lab | send={d} stop={d} chips={d} fast={} tools={} prompt={d} frame={d}",
