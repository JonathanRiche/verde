...
const Composer = palette.textArea(.{
    .x = 24,
    .y = 64,
    .width = 720,
    .height = 220,
    ...
    .corner_radius = 12,
    .border_width = 1.5,
    .placeholder_text = "Ask Palette to compose something...",
...
pub export fn palette_text_area_lab_main() c_int {
    run() catch |err| {
        std.debug.print("palette text_area_lab failed: {s}\n", .{@errorName(err)});
        return 1;
    };
    return 0;
}
...
pub fn run() !void {
    ...
    const window = try sdl.Window.create("Palette Text Area Lab", 800, 360, labWindowFlags());
    ...
    var title = try Title.init(allocator, "Palette TextArea");
    ...
    std.debug.print("palette text_area_lab started; close the window or press Ctrl+C to exit.\n", .{});
    defer std.debug.print("palette text_area_lab stopped.\n", .{});
    ...
    while (running) : (frame_index += 1) {
        ...
        while (sdl.pollEvent(&event)) {
            switch (event.type) {
                .quit => {
                    std.debug.print("palette text_area_lab received SDL quit.\n", .{});
                    running = false;
                },
                .window_close_requested => {
                    std.debug.print("palette text_area_lab received window close.\n", .{});
                    running = false;
                },
                else => {
        ...
        var presenter = palette.renderer.sdlFontRenderer(renderer, font, 16.0);
...
fn handleComposerEvent(_: ?*anyopaque, event: palette.TextAreaEvent) void {
    switch (event) {
...
fn handleComposerKey(_: ?*anyopaque, key: palette.TextAreaKey) palette.TextAreaAction {
...
fn updateWindowTitle(window: *sdl.Window, composer: Composer, command_count: usize, frame_index: usize) void {
    ...
    const title = std.fmt.bufPrintZ(
        &title_buffer,
        "Palette TextArea Lab | bytes={d} cursor={d} selection={d} commands={d} frame={d}{s}",
