//! Real font loading smoke test for Powder's SDL_ttf atlas.
...
pub export fn powder_font_loading_check_main() c_int {
    var gpa: std.heap.DebugAllocator(.{}) = .init;
    defer _ = gpa.deinit();
    run(gpa.allocator()) catch |err| {
        std.debug.print("powder font loading check failed: {s}\n", .{@errorName(err)});
        return 1;
    };
    return 0;
}
...
pub fn run(allocator: std.mem.Allocator) !void {
    for (desktop_fonts) |path| {
        var atlas = try powder.FontAtlas.init(allocator, path, 16.0);
        defer atlas.deinit();
