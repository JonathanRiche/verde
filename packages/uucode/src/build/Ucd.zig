...
fn readFileAlloc(allocator: std.mem.Allocator, file_path: []const u8, limit: usize) ![]u8 {
    var threaded: std.Io.Threaded = .init(allocator, .{});
    defer threaded.deinit();
    return try std.Io.Dir.cwd().readFileAlloc(threaded.io(), file_path, allocator, .limited(limit));
}
...
fn parseUnicodeData(allocator: std.mem.Allocator, unicode_data: []UnicodeData) !void {
    ...
    const content = try readFileAlloc(allocator, file_path, 1024 * 1024 * 10);
...
fn parseCaseFolding(
    ...
    const content = try readFileAlloc(allocator, file_path, 1024 * 1024);
    defer allocator.free(content);
...
fn parseDerivedCoreProperties(
    ...
    const content = try readFileAlloc(allocator, file_path, 1024 * 1024 * 2);
    defer allocator.free(content);
