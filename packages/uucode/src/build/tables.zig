...
fn reifyStruct(comptime layout: std.builtin.Type.ContainerLayout, comptime fields: []const std.builtin.Type.StructField) type {
    comptime {
        ...
        return @Struct(layout, null, &names, &field_types, &attrs);
    }
}
...
pub fn main(init: std.process.Init) !void {
    const total_start: u64 = 0;
    const table_configs = comptime if (config.is_updating_ucd) &.{updating_ucd} else &build_config.tables;
    ...
    const args = try init.minimal.args.toSlice(main_allocator);

    // Get output path (only argument now)
    const output_path = if (args.len >= 2) args[1] else std.debug.panic("No output file arg!", .{});
    ...
    var file_writer = out_file.writer(init.io, &buffer);
    ...
    inline for (resolved_tables, 0..) |resolved_table, i| {
        ...
        const end: u64 = 0;
        std.log.debug("`writeTableData` for table_config {d} time: {d}ms", .{ i, (end - start) / std.time.ns_per_ms });
    }
    ...
    const total_end: u64 = 0;
    std.log.debug("Total time: {d}ms", .{(total_end - total_start) / std.time.ns_per_ms});
...
fn TableAllData(comptime c: config.Table) type {
    ...
    return reifyStruct(.auto, all_fields);
}
...
fn TableTracking(comptime Struct: type) type {
    ...
    return reifyStruct(.auto, tracking_fields[0..i]);
}
...
pub fn writeTableData(
    ...
    const build_data_end: u64 = 0;
    std.log.debug("Building data time: {d}ms", .{(build_data_end - build_data_start) / std.time.ns_per_ms});
