...
pub const FieldEnum = blk: {
    ...
    var values: [fields.len]std.math.IntFittingRange(0, fields_len - 1) = undefined;
    for (fields, 0..) |field, index| {
        names[index] = field.name;
        values[index] = @intCast(field.value);
    }
    break :blk @Enum(std.math.IntFittingRange(0, fields_len - 1), .exhaustive, &names, &values);
};
