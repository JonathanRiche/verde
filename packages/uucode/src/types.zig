...
fn reifyUnion(comptime layout: std.builtin.Type.ContainerLayout, comptime Tag: ?type, comptime fields: []const std.builtin.Type.UnionField) type {
    comptime {
        var names: [fields.len][]const u8 = undefined;
        var field_types: [fields.len]type = undefined;
        ...
        return @Union(layout, Tag, &names, &field_types, &attrs);
    }
}
...
pub fn Data(comptime c: config.Table) type {
    ...
    return reifyStruct(if (c.packing == .@"packed") .@"packed" else .auto, &data_fields);
}
...
pub fn StructFromDecls(comptime Struct: type, comptime decl: []const u8) type {
    ...
    return reifyStruct(.auto, decl_fields[0..i]);
}
...
pub fn Union(comptime c: config.Field, comptime packing: config.Table.Packing) type {
    ...
    const InnerUnion = reifyUnion(if (packing == .@"packed") .@"packed" else .auto, if (packing == .@"packed") null else Tag, &fields);
