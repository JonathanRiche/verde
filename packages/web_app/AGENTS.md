...
- Serve static files only from the trusted built SPA. No permissive CORS, arbitrary directory browsing, or repository HTML/SVG previews. Authenticated `/api/file` and `/api/preview` serve workspace documents from chat links (PDF/office) only after `served_files.zig` opens a regular file beneath a held root directory descriptor (`openat2` with `RESOLVE_BENEATH | RESOLVE_NO_MAGICLINKS` on Linux; a no-follow component walk elsewhere) for a registered workspace path or repository binding root from daemon `workspace.list` (cached a few seconds); anything else is `403 path_outside_workspace`, and without a live daemon nothing is served. Keep the extension and size checks after that confinement. Serve from the authorized descriptor; office conversion uses a private temporary copy and cache reads reject symlinks and special files.
